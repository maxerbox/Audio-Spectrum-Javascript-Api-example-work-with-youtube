...
function checkMp3(id, callback) {
	visu._updateInfo('Converting video from server...');
		...
		if (xmlHttp.readyState == 4 && xmlHttp.status == 200)
			if (typeof callback != 'undefined') {
				...
				if (js.status == 'success') {
					callback(js.message);
					visu._updateInfo('Downloading music from server....');
				} else {
					onPlayerStateChange({
						data: 0
					});
					visu._updateInfo('Error when trying to convert the video from server. Server message: ' + js.message);
				}
...
function handleCurrentId(d) {
	...
	var jsonObject = {};
	jsonObject["status"] = {
		"currentid": playerObject.getVideoUrl().match(/[?&]v=([^&]+)/)[1]
	};
	connection.send(JSON.stringify(jsonObject));
